...
public class project_euler{
    ...
    //2
    public static int sumEvenFibNum (){
        int soma = 0;
        int n1 = 1;
        int n2 = 2;
        while (n2 <= 4000000){
    ...
    public static long largestProductGrid(){
        ...
        for (int i=0; i<20; i++){
            grid[i] = rows[i].trim().split("\\s+");
        }
        for (int i=0; i<20; i++){
            for (int j=0; j<20; j++){
                String temp = grid[i][j];
                numberGrid[i][j] = Integer.parseInt(temp);
            }
        }
        ...
        for (int[] row : numberGrid){
            long productRight = 0;
            for (int i=0; i<=16; i++){
                productRight = row[i]*row[i+1]*row[i+2]*row[i+3];
                if (productRight > result){
                    result = productRight;
                }
            }
        }
        for (int i=0; i<=16; i++){
            long productUp = 0;
            for (int j=0; j<20; j++){
                productUp = numberGrid[i][j]*numberGrid[i+1][j]*numberGrid[i+2][j]*numberGrid[i+3][j];
                if (productUp > result){
                    result = productUp;
                }
            }
        ...
        for (int i=0; i<=16; i++){
            ...
            for (int j=0; j<=16; j++){
                productDiagonalRight = numberGrid[i][j]*numberGrid[i+1][j+1]*numberGrid[i+2][j+2]*numberGrid[i+3][j+3];
                if (productDiagonalRight > result){
                    result = productDiagonalRight;
                }
            }
        }
        for (int i=0; i<=16; i++){
            for (int j=3; j<20; j++){
                long productDiagonalLeft = numberGrid[i][j]*numberGrid[i+1][j-1]*numberGrid[i+2][j-2]*numberGrid[i+3][j-3];
                if(productDiagonalLeft > result){
                    result = productDiagonalLeft;
                }
            }
        }
        return result;
    ...
    public static String largeSum (){
        ...
        String[] str = listOfNumbers.split("\\s+");
        for (int i=0; i<100; i++){
            numbers[i] = new BigInteger(str[i].strip());
        }
        BigInteger soma = numbers[0];
        for (int i=1; i<100; i++){
            soma = soma.add(numbers[i]);
        }
        return soma.toString().substring(0,10);
    }

    //14
    public static long getChainSize (long n){
        long size = 0;
        while (n > 1){
            size += 1;
            if (n%2 == 0){
    ...
    public static int longestCollatzSeq (int n){
        int max = 0;
    ...
    public static long powerDigitSum (){
        ...
        BigInteger pot2 = number.pow(1000);
        String str = pot2.toString();
        long sum = 0;
        for (char c : str.toCharArray()){
            int digit = c - '0';
            sum += digit;
